...
// .... non-mutable handle factories

/// @ingroup SoloAnyHandleAdvanced
/// @brief Safely build a non-mutable @c any_handle object,
/// building @em in-place an object of type @c T.
/// @pre @c T is not a reference type.
/// @pre @c T is constructible from @c args .
/// @param a_type_constructor_arguments_list A list of arguments from which the object can be constructed.
///
/// Example:
///
/// @code
///     // 1. use the global tag object stdex::in_place
///     // 2. make_any_handle<A>(stdex::in_place, args) will call the A's constructor which accepts arguments 'args': A{args}
...
/// @endcode
///
/// @note @c make_any_handle<T>(stdex::in_place, args) as the same effect than @c make_any_handle<T>(stdex::in_place_type<T>, args).
template < typename T, typename... Args >
any_handle make_any_handle( stdex::in_place_t, Args&&... a_type_constructor_arguments_list );

/// @ingroup SoloAnyHandleAdvanced
/// @brief Safely build a non-mutable @c any_handle object,
/// building @em in-place an object of type @c U @em where @c U* is convertible to @c T*.
/// @pre @c T is not a reference type.
/// @pre @c U is not a reference type.
...
///     // 2. make_any_handle<A>(stdex::in_place_type<B>, args) will
...
///     auto y = make_any_handle<A>(stdex::in_place_type<B>, args);
///     assert(y.use_count() == 1);
///     assert(y.type() == typeid(A));
///     assert(y.is_mutable() == false);
///
/// @endcode
/// @note Use @c make_any_handle<T>(stdex::in_place, args) instead of @c make_any_handle<T>(stdex::in_place_type<T>, args).
template < typename T, typename U, typename... Args >
any_handle make_any_handle( stdex::in_place_type_t<U>, Args&&... a_type_constructor_arguments_list );

/// @ingroup SoloAnyHandleAdvanced
/// @brief Safely build a non-mutable @c any_handle object
/// from an @em observer pointer (i.e. a non-owned pointer) to an already-built object of type @c T.
/// @pre @c T is not a reference type.
...
/// auto x = T{...};
/// {
///     auto y = make_any_handle(stdex::make_observer(&x));
///     assert(y.use_count() == 1);
...
/// @endcode
///
/// @note The main point is that the shared pointer built around the object pointed by @c op won't delete the pointee object after the shared count felt to zero.
template < typename T >
any_handle make_any_handle( stdex::observer_ptr<T> const &a_non_owned_pointer_to_copy );

/// @ingroup SoloAnyHandleAdvanced
/// @brief Safely build a non-mutable @c any_handle object
/// from a pointer to an already-built object of type @c T and a @em customized finalizer.
/// @pre @c T is not a reference type.
/// @param a_raw_pointer_to_copy A raw pointer of type @c T.
...
///     auto y = make_any_handle(&x , [](A *p){ std::cout << "delete x" << std::endl; } );
///     assert(y.use_count() == 1);
///     assert(y.pointer().get() == &x);
///     assert(y.type() == typeid(A));
///     assert(y.is_mutable() == false);
...
///     assert(y.use_count() == 1);
///     assert(y.pointer().get() == &z);
///     assert(y.type() == typeid(A));
///     assert(y.is_mutable() == false);
/// }
/// @endcode
template < typename T, typename F >
any_handle make_any_handle( T *a_raw_pointer_to_copy, F &&a_callable_finalizer );

// .... mutable handle factories
...
template < typename T, typename... Args >
inline any_handle
make_any_handle( stdex::in_place_t, Args&&... a_type_constructor_arguments_list )
{
    static_assert(!std::is_reference<T>::value, "T should not be a reference type");

    // call the first factory :
    return make_any_handle( std::make_shared<T const>(std::forward<Args>(a_type_constructor_arguments_list)...) );
}

/// @ingroup SoloAnyHandleAdvanced
template < typename T, typename U, typename... Args >
inline any_handle
make_any_handle( stdex::in_place_type_t<U>, Args&&... a_type_constructor_arguments_list )
{
    static_assert(!std::is_reference<T>::value, "T should not be a reference type");
    static_assert(!std::is_reference<U>::value, "U should not be a reference type");
    static_assert(std::is_convertible<U const*,T const*>::value, "U const * should be convertible to T const *");

    // call the first factory (std::shared_ptr<U const> will be implicitly converted to std::shared_ptr<T const>):
    return make_any_handle<T const>( std::make_shared<U const>(std::forward<Args>(a_type_constructor_arguments_list)...) );
}

/// @ingroup SoloAnyHandleAdvanced
template < typename T >
inline any_handle
make_any_handle( stdex::observer_ptr<T> const &a_non_owned_pointer_to_copy )
{
    static_assert(!std::is_reference<T>::value, "T should not be a reference type");

    // call the first factory :
    return make_any_handle( std::shared_ptr<T const>{ a_non_owned_pointer_to_copy.get(), [](T*){} } );
}

/// @ingroup SoloAnyHandleAdvanced
template < typename T, typename F >
inline any_handle
make_any_handle( T *a_raw_pointer_to_copy, F &&a_callable_finalizer )
{
    static_assert(!std::is_reference<T>::value, "T should not be a reference type");
    static_assert(boost::hof::is_invocable<F,std::remove_cv_t<T>*>::value, "Bad finalizer signature");

    // call the first factory :
    return make_any_handle( std::shared_ptr<T const>{ a_raw_pointer_to_copy, std::forward<F>(a_callable_finalizer) } );
}
